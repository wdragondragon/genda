...
public class AchievementListener extends AbstractAction {
  ...
  JTextPane wenben;
  double sudu, second, length, Keymethod, right, left, KeyNumber, mistake, deleteNumber,
      deleteTextNumber, repeat, fengzhi, yingdasudu, yingdamistake;
      ...
      try { // 判断对战中
        if (!battleClient.socket.isClosed()) {
          if (Window.reducesudu.getText() != "")
            battleSend.out.writeUTF("%" + ReadyListener.BeganSign + "%"
                + "%" + RegexText.duan1 + "#" + Window.wenben.getText() + "%"
                + String.valueOf(sudu - 30 * battleSend.Mistake
                      - Integer.parseInt(Window.reducesudu.getText()))
                + "%" + Login.zhanghao.getText());
          else
            battleSend.out.writeUTF("%" + ReadyListener.BeganSign + "%"
                + "%" + RegexText.duan1 + "#" + Window.wenben.getText() + "%"
                + String.valueOf(sudu - 30 * battleSend.Mistake) + "%" + Login.zhanghao.getText());
  ...
  public String getGeshi() {
    ...
    if (Window.Pattern == 1) {
      ...
      sudu = Window.gendaListener.comp.getSpeed(
          Window.gendaListener.str1.length(), (int) (Window.gendaListener.mistake));
      ...
      yingdanomistakesudu =
          String.format("%.2f", Window.gendaListener.comp.getSpeed(temp.length, 0));
    } else {
      nomistakesudu = String.format(
          "%.2f", Window.gendaListener.comp.getSpeed(Window.dazi.getText().length(), 0));
      sudu = Window.gendaListener.comp.getSpeed(
          Window.gendaListener.str1.length(), (int) (Window.gendaListener.mistake));
    }
    nomistakesudu = String.format(
        "%.2f", Window.gendaListener.comp.getSpeed(Window.gendaListener.str1.length(), 0));

    Keyaccuracy =
        (KeyNumber - deleteNumber * 2
            - deleteTextNumber
                * (1.0 * Window.tipschange.alllength / QQZaiwenListener.wenbenstr.length()))
        / KeyNumber;
    dacilv = ((double) (gendaListener.daciall)
        / (QQZaiwenListener.wenbenstr.length() + deleteTextNumber));

    gendageshi = "第" + RegexText.duan1;

    if (mistake == 0)
      gendageshi += "段 速度" + String.format("%.2f", sudu) + " 击键"
          + String.format("%.2f", KeyNumber / second) + " 码长"
          + String.format("%.2f", KeyNumber / length) + " 标顶理论"
          + String.format("%.2f", Tips.dingalllength / QQZaiwenListener.wenbenstr.length())
          + " 文章难度" + String.format("%.2f", Tips.dengji) + " 字数" + (int) (length) + " 回改"
          + (int) (deleteTextNumber) + " 退格" + (int) (deleteNumber) + " 错字" + (int) (mistake)
          + " 键数" + (int) (KeyNumber) + " 选重" + (int) (repeat) + " 键准"
          + String.format("%.2f", Keyaccuracy * 100) + "% 键法"
          + String.format("%.2f", Keymethod * 100) + "%(左" + String.valueOf((int) left) + ":右"
          + String.valueOf((int) right) + ":空格" + String.valueOf(space) + ")"
          + " 打词率" + String.format("%.2f", dacilv * 100) + "% 选重率"
          + String.format("%.2f", repeat / length * 100) + "% 拖拉机跟打器 " + Login.banben + " "
          + Example.systemname + "版 ";
    else
      gendageshi += "段 速度" + String.format("%.2f", sudu) + "/" + nomistakesudu + " 击键"
          + String.format("%.2f", KeyNumber / second) + " 码长"
          + String.format("%.2f", KeyNumber / length) + " 标顶理论"
          + String.format("%.2f", Tips.dingalllength / QQZaiwenListener.wenbenstr.length())
          + " 文章难度" + String.format("%.2f", Tips.dengji) + " 字数" + (int) (length) + " 回改"
    ...
    if (Window.Pattern == 1)
      gendageshi +=
          " 看打版 " + AchListener.lookplay + " 校验" + DoCheck.buildcheckstr(check, "kanda");
    else if (Window.Pattern == 2)
      if (mistake == 0)
        gendageshi += "英打版 速度:" + String.format("%.2f", yingdasudu) + " 单词" + temp.length
            + " 错" + (int) yingdamistake + " 校验" + DoCheck.buildcheckstr(check, "yingda");
      else
        gendageshi += "英打版 速度:" + String.format("%.2f", yingdasudu) + "/" + yingdanomistakesudu
            + " 单词" + temp.length + " 错" + (int) yingdamistake + " 校验"
    ...
    try {
      if (Window.everydaySign) {
        sendsaiwen();
        Login.out.writeUTF(KeyPassword.convertMD5("扣扣发送成绩" + Login.zhanghao.getText()
            + "已打今日赛文\n成绩：速度" + String.format("%.2f", sudu) + " 击键"
            + String.format("%.2f", KeyNumber / second) + " 码长"
    ...
    try { // 判断对战中
      if (!battleClient.socket.isClosed()) {
        gendageshi = gendageshi + " 让速" + Window.reducesudu.getText() + " 重打"
            + battleSend.Mistake + " 最终速度(显示速度-重打*30-让速)"
            + String.format("%.2f",
                  sudu - 30 * battleSend.Mistake - Integer.parseInt(Window.reducesudu.getText()));
  ...
  public void sendsaiwen() {
    if (Login.dengluSign == 1) {
      String message = "成绩"
          + "%" + Login.zhanghao.getText() + "%" + String.format("%.2f", sudu) + "%"
          + String.format("%.2f", KeyNumber / second) + "%"
          + String.format("%.2f", KeyNumber / length) + "%" + (int) (length) + "%"
          + (int) (deleteTextNumber) + "%" + (int) (deleteNumber) + "%" + (int) (mistake) + "%"
          + (int) (repeat) + "%" + String.format("%.2f", Keyaccuracy * 100) + "%"
          + String.format("%.2f", Keymethod * 100) + "%" + String.format("%.2f", dacilv * 100) + "%"
  ...
  public void sendhistory() {
    if (Login.dengluSign == 1) {
      String message = "历史"
          ...
          + (int) (repeat) + "%" + String.format("%.2f", Keyaccuracy * 100) + "%"
          + String.format("%.2f", Keymethod * 100) + "%" + String.format("%.2f", dacilv * 100) + "%"
          + String.valueOf(GendaListener.comp.getSecond()) + "%" + QQZaiwenListener.wenbenstr + "%"
          + String.valueOf(RegexText.duan1) + "%" + String.format("%.2f", Tips.dengji); // 14
      message = KeyPassword.convertMD5(message);
      try {
        Login.out.writeUTF(message);
      } catch (IOException e) {
