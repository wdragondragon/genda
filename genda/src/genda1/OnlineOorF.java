...
public class OnlineOorF implements ActionListener {
  ...
  public void actionPerformed(ActionEvent arg0) {
    ...
    if (one.isVisible()) {
      ...
      if (win.getHeight() < (tableweizhi1 + accept.getHeight() + 45))
        win.setSize(win.getWidth(),
            win.getHeight() + accept.getHeight() - ((win.getHeight() - (tableweizhi1 + 5))) + 45);
      else
  ...
  public void setRoomNum(JTextField reducesudu, JButton one, JButton two, JButton three,
      JButton four, JButton five, JButton six, JButton seven, JButton eight, JButton link,
      JButton breaklink, JScrollPane accept, JButton ready, JButton score, JScrollPane communion,
      Window win) {
    this.one = one;
    this.two = two;
    this.three = three;
