...
public class Clip extends Thread {
    ...
    try {
      ...
      try {
        ...
        out.writeUTF("%" + ReadyListener.BeganSign + "%"
            + "%" + RegexText.duan1 + "#" + QQZaiwenListener.wenbenstr + "%0"
  ...
  void setgendaSign() throws InterruptedException {
    ...
    Window.gendaListener.zishu.setText("跟打完毕字数:" + Window.gendaListener.str2.length() + "/错"
        + Window.gendaListener.mistake); // 字数显示
    Window.gendaListener.Keylength.setText(String.format("%.2f",
        Window.gendaListener.KeyNumber / Window.gendaListener.str2.length())); // 码长显示
    ...
    AchievementListener.setClipboardString(
        Window.gendaListener.achievementlistener.getGeshi()); // 将成绩放入剪贴板
    ...
    double nextKeyLength =
        Double.valueOf(String.valueOf(Window.acticle.spinnerKeyLength.getValue()));
    double speed = Window.gendaListener.getKeyNumber();
    double keySpeed = Window.gendaListener.KeyNumber / GendaListener.comp.getSecond();
    double keyLength = Window.gendaListener.KeyNumber / GendaListener.str1.length();
    if ((nextSpeed == 0 || speed >= nextSpeed) && (nextKey == 0 || keySpeed >= nextKey)
