...
public class GendaListener implements DocumentListener, KeyListener {
  ...
  public void keyTyped(KeyEvent e) {
    ...
    try {
      ...
      for (n = 0; n < str1.length(); n++) { // 统计错误字数，向文本框添加字体
        if (c1[n] != c2[n]) {
          mistake++;
          String mistakeStr =
              "\"" + String.valueOf(c2[n]) + "\"在第" + String.valueOf(n + 1) + "个字\n";
      ...
      allnumber.setText("总:" + String.valueOf(Window.fontallnum)
    ...
    if (Window.Pattern == 2) {
      try {
        if (dazi.getText().length() == QQZaiwenListener.wenbenstr.length())
          return;
  ...
  public void compdaci(char c) {
    if (str1 != "" && str1.length() >= length) {
      ...
      if (compdazi.getTimeOne() - compdazi.getTimeTwo() < 50) {
        ...
        dacilist.add(temp + ":" + citime + ":" + String.format("%.2f", sudu) + ":"
            + String.format("%.2f", KeyNumber / getSecond()) + ":"
            + String.format("%.2f", compshushi()));
      } else if (daci == 0) {
        dacilist.add(String.valueOf(c) + ":" + comp.getSecond() + ":" + String.format("%.2f", sudu)
            + ":" + String.format("%.2f", KeyNumber / getSecond()) + ":"
            + String.format("%.2f", compshushi()));
      }
    }
        ...
        for (n = 0; n < str2.length(); n++) { // 统计错误字数，向文本框添加字体
          if (c1[n] != c2[n])
            JTextPaneChange.insertDoc(
                JTextPaneChange.styledDoc, String.valueOf(c2[n]), "红", wenben);
          else
            JTextPaneChange.insertDoc(
                JTextPaneChange.styledDoc, String.valueOf(c2[n]), "黑", wenben);
        }
      } catch (Exception e) {
        n = 0;
    ...
    try {
      ...
      if (n > dangyenum * (fenye - 1) + Integer.valueOf(SetFrame.readyFont.getText())) {
        ...
        for (; n < tempready; n++) {
          ...
          JTextPaneChange.createStyle("预读", JTextPaneChange.styledDoc, Window.fontSize, 0, 0, 0,
              wenben.getBackground(), Window.family, wenben.getBackground());
          JTextPaneChange.insertDoc(
              JTextPaneChange.styledDoc, String.valueOf(c2[n]), "预读", wenben);
        }
      }
          ...
          if (ejianmanum.indexOf(n) != -1) {
            if (ejianmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "粉粗", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "粉", wenben);
              }
            }
            n = tail;
            ...
            if (ciejianmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "粉粗斜", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "粉斜", wenben);
              }
            }
            n = tail;
            ...
            if (cisjianmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "蓝粗斜", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "蓝斜", wenben);
              }
            }
            n = tail;
            ...
            if (sjianmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "蓝粗", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "蓝", wenben);
              }
            }
            n = tail;
            ...
            if (ciquanmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "绿粗斜", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "绿斜", wenben);
              }
            }
            n = tail;
            ...
            if (quanmanum.indexOf(n) % 2 == 0)
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "绿粗", wenben);
              }
            else {
              for (int k = n; k <= tail; k++) {
                JTextPaneChange.insertDoc(
                    JTextPaneChange.styledDoc, String.valueOf(c2[k]), "绿", wenben);
              }
            }
            n = tail;
            ...
            JTextPaneChange.insertDoc(
                JTextPaneChange.styledDoc, String.valueOf(c2[n]), "灰", wenben);
          }
        }
      }
    } catch (Exception ex) {
