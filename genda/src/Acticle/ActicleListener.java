...
public class ActicleListener implements TreeSelectionListener, ActionListener {
  ...
  public static void showwen() {
    ...
    // fontweizhi += fontnum;
    win.sendwen.setText(String.valueOf(fontweizhi) + "/" + String.valueOf(all.length()) + ":"
        + String.format("%.2f", (double) fontweizhi * 100 / all.length()) + "%");
  }
    ...
    try {
      ...
      try {
        ...
        out.writeUTF("%" + ReadyListener.BeganSign + "%"
            + "%" + RegexText.duan1 + "#" + Window.wenben.getText() + "%0"
  ...
  public void chouqu(String model) {
    if (SendWenben.sendwenSign3 == 1) {
      ...
      if (SetFrameQianshuiListener.qianshui == 0)
        ShareListener.send();
    } else if (model.equals("抽取模式发文") || SendWenben.sendwenSign2 == 1) {
      getNumber();
  ...
  public static String randomCommon(String wen, int n) {
    ...
    for (int i = 0; i < result.length; i++) resultstr.append(String.valueOf(c[result[i]]));
