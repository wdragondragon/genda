...
public class battleReadThread extends Thread {// 读取服务器发来信息的线程
	...
	public battleReadThread(Socket socket, JTextArea accept2,
			GendaJindutiao gendajindu) {
		this.socket = socket;
		this.accept = accept2;
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (num[0] == 0)
					continue;
				int sign = Integer.parseInt(message.substring(num[0],
						num[1] - 1));
				...
				if (!wenbentemp.equals("无") && !wenbentemp.equals("")
						&& wenbentemp != null) {
				...
				Window.communion.setCaretPosition(Window.communion.getText()
						.length());
			}
		} catch (IOException e) {
			// e.printStackTrace();
