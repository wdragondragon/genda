...
public class getDatesaiwen implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (Window.everydaySign) {
      JOptionPane.showMessageDialog(new JTextArea(), "正在进行赛文");
      return;
    }
    int n = JOptionPane.showConfirmDialog(null,
        "在跟打每日赛文时，重打系统移除，过程中不可载入别的文章，不可退出登录，不可退出跟打器等等一切尝试二次跟打的行为，否则今日成绩归零\n请检查自身输入法调配，做好防止任何弹窗的准备，准备好了吗?点击“是”后将立即开始计时",
        "赛文提示", JOptionPane.YES_NO_OPTION);
    if (n == JOptionPane.YES_OPTION) {
      ...
      try {
        ...
        if (message.equals("已打过")) {
          JOptionPane.showMessageDialog(new JTextArea(), "你今天已打过赛文");
          return;
        } else if (message.equals("无赛文")) {
          JOptionPane.showMessageDialog(new JTextArea(), "今日无赛文");
          return;
        }
        SendWenben.title = "拖拉机每日赛文-作者：随机生成";
        QQZaiwenListener.wenbenstr = RegexText.huanfu(RegexText.qukong(message));
