...
public class historyselect implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    historyFrame.yemashow.setText(
        String.valueOf(historyFrame.dangqian + 1) + "/" + String.valueOf(historyFrame.yenum + 1));
  }
  void lianjie() {
    try {
    ...
    try {
      while (true) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(new JTextArea(), "输入错误");
          return;
        }
  ...
  void requestdate() {
    try {
      ...
      if (historyFrame.num == 0) {
        JOptionPane.showMessageDialog(new JTextArea(), "无记录");
        return;
      }
      historyFrame.aver();
      historyFrame.showtable();
