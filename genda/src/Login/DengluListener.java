	...
	public void denglu(){
		if(Login.dengluSign==0){
			try{
				...
				String message = "%1%"+Login.zhanghao.getText()+"%"+Login.mima.getText()+"%-999"+"%无"+"%无";
				...
				int misnum = Integer.parseInt(message.substring(num[3]));
				if(i==1){
					Login.dengluSign = 1;
			...
			try {
				...
				Login.socket = new Socket(Window.IP,1232);
				Login.out = new DataOutputStream(Login.socket.getOutputStream());
