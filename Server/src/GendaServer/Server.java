...
public class Server extends Thread {
              ...
              for (int i = 0; i < socketList.size(); i++) { //发送给所有连接的客户端
                ...
                if (socketList.lastIndexOf(socketList.get(i)) == 0) {
                  try {
                    ...
                    out.writeUTF(
                        "%0%无%无%无%无%房间已满，请准备开始,载文后等待对方跟打框变空白\n对方跟打框变白时，对方载文成功，即将开始跟打\n你是房主\n%无");
  ...
  class RWThread
      extends Thread { //接收和发送消息的线程   发送1 房主 2跟打进度 3文本  4计分 5系统 6开始
    ...
    public void run() {
      ...
      try {
        ...
        while (true) {
            ...
            if (began == 1) {
              ...
              for (int i = 0; i < socketList.size(); i++) { //发送给所有连接的客户端;
                if (socket != socketList.get(i)) {
                  out = new DataOutputStream(socketList.get(i).getOutputStream());
                  out.writeUTF("%" + String.valueOf(mumber[socketList.lastIndexOf(socket)]) + "%无"
                      + "%无"
                      + "%无"
                      + "%无%" + system + "%无");
                }
          ...
          sudu[socketList.lastIndexOf(socket)] =
              Double.parseDouble(message.substring(num[3], num[4] - 1));
          ...
          if (sudu[0] > sudu[1] && sudu[0] != 0 && sudu[1] != 0) { //宣布赢者，重置速度
            ...
            for (int i = 0; i < socketList.size(); i++) { //发送给所有连接的客户端;
              out = new DataOutputStream(socketList.get(i).getOutputStream());
              out.writeUTF("%" + String.valueOf(mumber[socketList.lastIndexOf(socket)]) + "%"
                  + jindu + "%" + wenben + "%" + name[0] + ":" + name[1] + "/%"
                  + String.valueOf(win[0]) + ":" + String.valueOf(win[1]) + "%无"
                  + "%无");
            }
            ...
            if (began == 1) {
              ...
              for (int i = 0; i < socketList.size(); i++) { //发送给所有连接的客户端;
                if (socket != socketList.get(i)) {
                  out = new DataOutputStream(socketList.get(i).getOutputStream());
                  out.writeUTF("%" + String.valueOf(mumber[socketList.lastIndexOf(socket)]) + "%"
                      + "%" + wenben + "%" + name[0] + ":" + name[1] + "/%" + String.valueOf(win[0])
                      + ":" + String.valueOf(win[1]) + "%无"
                      + "%无");
                }
              }
            }
        ...
        System.out.print("玩家离开" + portNum / 1111 + "房间" + portNum / 1111 + "剩余"
            + (--socketSum) + "玩家\r"); //客户断开后sockeSum减一
