...
public class RWThread148 extends Thread {
  ...
  public void run() {
    ...
    try {
      ...
      if (dengluSign == 1) {
        try {
          System.out.println("最后字数:" + recordNumlast + ":" + recordrightnum + ":" + recordmisnum
              + ":" + recorddatenumlast);
          //					if(recordNumlast<0)
          //						recordNumlast = 0;
  ...
  public void changeRecord(int recordnum, int recordrightnum, int recordmisnum,
      int recorddatenumlast) throws SQLException {
    String sql =
        "update client set num =?,rightnum=?,misnum=?,datenum=? where username=?"; //搜索存在用户名，并改民字数
    ...
    ptmt.execute();
    System.out.println(
        recordnum + " " + recordrightnum + " " + recordmisnum + " " + recorddatenumlast);
  }
  ...
  public void zhuce(String username, String password) throws SQLException {
    ...
    if (rs.next()) { //检测用户名是否被使用过
      ...
      try {
        ...
        System.out.print(username + "通知相同用户名失败\r");
      }
    } else {
      ...
      try {
        ...
        System.out.print(username + "通知注册成功失败\r");
      }
    }
  }
  //用户登录
    ...
    try {
      ...
      if (!rs.next()) { //检验账号是否存在
        ...
        try {
          ...
          System.out.print(username + "发送登录失败失败\r");
        }
        return;
      }
      ptmt = Recordnum.con.prepareStatement(sql);
      ...
      if (rs.next()) { //验证密码是否正确
        ...
        try {
          ...
          String message = "%1%" + String.valueOf(num) + "%" + String.valueOf(rightnum) + "%"
              + String.valueOf(misnum) + "%" + String.valueOf(datenum) + "%" + email;
          System.out.println(message);
          out.writeUTF(message);
          dengluSign = 1;
          System.out.print(username + "登录成功！\r");
        } catch (IOException e) {
          System.out.print(username + "发送登录成功失败\r");
        }
      } else {
